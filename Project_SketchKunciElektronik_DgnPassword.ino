...
Keypad ridho = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
void setup() { 
  ...
  delay(500);
  lcd.clear();
  ridho.addEventListener(keypadEvent);
}
void loop() {
  pinMode(led, OUTPUT);
   if (a==0) { //state awal
  ...
  ridho.getKey(); //membaca keypad
}
void keypadEvent(KeypadEvent eKey){ 
  switch (ridho.getState()){
